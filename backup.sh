...
psql $POSTGRES_HOST_OPTS -d $POSTGRES_DATABASE -w -c "COPY (WITH t2 AS (select row_to_json(t) as js from cans.i_18_n t), t1 AS (SELECT ARRAY_TO_JSON(array_agg(js)) as \"i_18_n\" FROM t2) SELECT row_to_json(t1) FROM t1) TO STDOUT WITH ENCODING 'UTF-8'" | sed 's/\\\\/\\/g' | aws $AWS_ARGS s3 cp - s3://$S3_BUCKET/$S3_PREFIX/row_i_18_n_$(date +"%Y-%m-%d").json || exit 2

psql $POSTGRES_HOST_OPTS -d $POSTGRES_DATABASE -w -c "COPY (WITH t2 AS (select row_to_json(t) as js from cans.instrument t), t1 AS (SELECT ARRAY_TO_JSON(array_agg(js)) as \"instrument\" FROM t2) SELECT row_to_json(t1) from t1) TO STDOUT WITH ENCODING 'UTF-8'" | sed 's/\\\\/\\/g' | aws $AWS_ARGS s3 cp - s3://$S3_BUCKET/$S3_PREFIX/row_instrument_$(date +"%Y-%m-%d").json || exit 2

psql $POSTGRES_HOST_OPTS -d $POSTGRES_DATABASE -w -c "COPY (WITH t2 AS (select row_to_json(t) as js from cans_report.person_v t), t1 AS (SELECT ARRAY_TO_JSON(array_agg(js)) as \"person_v\" FROM t2) SELECT row_to_json(t1) FROM t1) TO STDOUT WITH ENCODING 'UTF-8'" | sed 's/\\\\/\\/g' | aws $AWS_ARGS s3 cp - s3://$S3_BUCKET/$S3_PREFIX/row_person_$(date +"%Y-%m-%d").json || exit 2

psql $POSTGRES_HOST_OPTS -d $POSTGRES_DATABASE -w -c "COPY (WITH t2 AS (select row_to_json(t) as js from cans_report.assessment_domain_v t), t1 AS (SELECT ARRAY_TO_JSON(array_agg(js)) as \"assessment_domain_v\" FROM t2) SELECT row_to_json(t1) from t1) TO STDOUT WITH ENCODING 'UTF-8'" | sed 's/\\\\/\\/g' | aws $AWS_ARGS s3 cp - s3://$S3_BUCKET/$S3_PREFIX/row_domain_$(date +"%Y-%m-%d").json || exit 2
